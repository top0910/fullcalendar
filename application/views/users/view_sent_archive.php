...
<?php $subject = substr($email['email_subject'], 0, 40); ?>
    <?= $subject; ?></a>                                                                    
                ...
                <td> 
                    <a class="btn btn-warning btn-sm" href="<?= base_url('user/sent_revert_archive?id='.$email['id']) ?>" onclick="if(confirm('Are your sure want to archive this email !'));else{ return false}"><i class="mdi mdi-sync" style="color: white;"></i></a>
                </td> 
                                                        </tr>
                                                     <?php endforeach; ?>
                                                     <?= form_close(); ?>
