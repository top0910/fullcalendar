                  ...
                  <div class="buttons-box clearfix">
                    <?php if ($this->current_user_role == 0) :?>
                      ...
                      <?php elseif ($this->current_user_id == 67): ?>
                        <button class="btn btn-success" disabled="true" type="">Update Status </button> 

                      <?php else: ?>
                        <button class="btn btn-success" type="">Update Status </button> 
                      <?php endif; ?>             
                  </div>
                  <?= form_close() ?>
              </div>
...
<!-- ****************************************** EDIT ENDS *************************-->
  ...
  <td><a class="btn btn-danger btn-sm" href="<?= base_url('user/request_delete?id=' . $value['id']) ?>" onclick="if(confirm('Are your sure want to delete !'));else{ return false}"><span class="glyphicon glyphicon-trash"></span> Delete</a></td>
      ...
      <?php elseif ($this->current_user_id == 67): ?>
  <td><button disabled='true' class="btn btn-danger btn-sm" href="<?= base_url('user/request_delete?id=' . $value['id']) ?>" onclick="if(confirm('Are your sure want to delete !'));else{ return false}"><span class="glyphicon glyphicon-trash"></span> Delete</button></td> 
